...
//
// Guarded by hand rather than via `server-only`: this must import in any
// server runtime (Next.js, scripts, cron handlers), and server-only's
// react-server export condition throws outside React.
...
// No database name: MONGODB_URI carries no path, so this is the driver's
// implicit default. Deliberately the same call @repo/database makes, so
// better-auth's collections (user, session, organization, member, …) land
// beside `subscribers`/`digests` instead of in a database nothing else reads.
const db = client.db();
